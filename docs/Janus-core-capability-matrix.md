...
| OPS-05 | Grafana dashboard：backlog、online agents、DLQ、latency、errors、policy/budget/routing blocks | P1 | Covered | dashboard JSON `jq`、Grafana provisioning smoke、`make smoke-grafana-panels` 静态 cross-check（dashboard PromQL 中每个 `janus_*` 指标必须在 `server/internal/metrics/metrics.go` 注册；histogram 自动展开 `_bucket`/`_sum`/`_count`；orphan 即 fail），可选 live `/metrics` 校验 | 新增/重命名指标必须同步 dashboard JSON，否则 smoke-grafana-panels gate fail。 |
